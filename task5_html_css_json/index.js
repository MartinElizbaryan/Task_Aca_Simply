...
const renderSystemsContent = (allSystems) => {
  ...
  const table = document.querySelectorAll(".system-table");
  const nestedTable = document.querySelectorAll(".systems-table_nested");

  systems.forEach((system, i) => {
    table[i].querySelector(".system-name").innerHTML +=
      system.system_name;
    table[i].querySelector(".system-id").innerText = system.id;
    table[i].querySelector(".system-date").innerText =
      system.created_date;
    table[i].querySelector(".system-licenses").innerText =
      system.active_licenses;
  });
  subsystems.forEach((subsystem, i) => {
    if (nestedTable[i]) {
      nestedTable[i].querySelector(".nested-system-name").innerText =
        subsystem.licenses;
      nestedTable[i].querySelector(".nested-system-date").innerText =
        subsystem.expires;
    }
  });
};
